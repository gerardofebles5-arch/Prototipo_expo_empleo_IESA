...
st.set_page_config(
    page_title="Portfolio Elite | ExpoEmpleo IESA 2026",
    page_icon="🚀",
    layout="wide",
...
def abc_analysis(values):
    ...
    for val in sorted_vals:
        ...
        if pct <= 0.8: categories.append('A')
        elif pct <= 0.95: categories.append('B')
        else: categories.append('C')
    return categories
...
with st.sidebar:
    st.markdown("""
     <div style="background: linear-gradient(135deg, #6366f1, #06b6d4); padding: 24px; border-radius: 16px; color: white; text-align: center; margin-bottom: 20px;">
         ...
         <p style="margin: 4px 0; font-size: 0.9rem; opacity: 0.9;">Estudiante de Ingeniería de Sistemas - 9no Semestre</p>
    ...
    selected = st.radio("Navegación", [
        "🏠 Dashboard", "🛡️ ShieldVZLA Elite", "📈 TradeGuard Elite", 
...
if selected == "🏠 Dashboard":
    ...
    cols = st.columns(5)
    metrics = [
        ("Módulos", "5", "Enterprise"), ("Líneas Código", "2,500+", "Python 3.12"),
        ("Empresas", "25+", "IESA 2026"), ("Tecnologías", "8", "Stack Elite"), ("Nivel", "Elite", "2026")
    ]
...
# ================================================================================
    ...
    with tab2:
        ...
        mitre_data = pd.DataFrame({
            ...
            'Técnica': ['T1490', 'T1005', 'T1021', 'T1547', 'T1070'],
            'Detecciones': [12, 8, 5, 3, 7],
            'Alertas': [3, 2, 1, 0, 1]
        })
        fig = px.bar(mitre_data, x='Táctica', y=['Detecciones', 'Alertas'], barmode='group', title='Cobertura MITRE ATT&CK v14')
        st.plotly_chart(fig, use_container_width=True)
        st.dataframe(mitre_data, use_container_width=True, hide_index=True)

    with tab3:
        st.markdown("<h4>IOC Database</h4>", unsafe_allow_html=True)
    ...
    with tab4:
        st.markdown("<h4>Forensics Timeline</h4>", unsafe_allow_html=True)
        conn = get_db()
        df = pd.read_sql_query("SELECT * FROM security_alerts ORDER BY created_at DESC", conn)
        conn.close()
        st.dataframe(df[['alert_id', 'severity', 'mitre_technique', 'target_system', 'status', 'created_at']], use_container_width=True, hide_index=True)
...
# ================================================================================
    ...
    df_ohlc = pd.DataFrame({
        'Date': dates, 'Open': prices * (1 + np.random.normal(0, 0.001, days)),
        'High': prices * (1 + abs(np.random.normal(0, 0.015, days))),
        'Low': prices * (1 - abs(np.random.normal(0, 0.015, days))), 'Close': prices
    }).set_index('Date')
    ...
    df_ohlc['MACD'], df_ohlc['MACD_Signal'], df_ohlc['MACD_Hist'] = macd, signal, hist
    upper, middle, lower = calc_bollinger(df_ohlc['Close'])
    df_ohlc['BB_Upper'], df_ohlc['BB_Middle'], df_ohlc['BB_Lower'] = upper, middle, lower
    ...
    with tab1:
        ...
        with col1:
            fig = go.Figure()
            fig.add_trace(go.Candlestick(x=df_ohlc.index, open=df_ohlc['Open'], high=df_ohlc['High'], low=df_ohlc['Low'], close=df_ohlc['Close'], name="Price"))
            if show_sma50: fig.add_trace(go.Scatter(x=df_ohlc.index, y=df_ohlc['SMA50'], name="SMA 50", line=dict(color='orange', width=1)))
            if show_sma200: fig.add_trace(go.Scatter(x=df_ohlc.index, y=df_ohlc['SMA200'], name="SMA 200", line=dict(color='purple', width=1)))
            if show_bb:
                fig.add_trace(go.Scatter(x=df_ohlc.index, y=df_ohlc['BB_Upper'], name="BB Upper", line=dict(color='gray', width=1, dash='dash')))
                fig.add_trace(go.Scatter(x=df_ohlc.index, y=df_ohlc['BB_Lower'], name="BB Lower", line=dict(color='gray', width=1, dash='dash')))
            fig.update_layout(title="Análisis Técnico Avanzado", yaxis_title="Precio", xaxis_title="Fecha", height=500, template="plotly_white")
            st.plotly_chart(fig, use_container_width=True)

            latest = df_ohlc.iloc[-1]
            c1, c2, c3 = st.columns(3)
            c1.markdown(f"<span class='badge {'badge-green' if latest['SMA50'] > latest['SMA200'] else 'badge-red'}'>{'🟢 GOLDEN CROSS' if latest['SMA50'] > latest['SMA200'] else '🔴 DEATH CROSS'}</span>", unsafe_allow_html=True)
            c2.markdown(f"<span class='badge {'badge-red' if latest['RSI'] > 70 else 'badge-green' if latest['RSI'] < 30 else 'badge-blue'}'>{'📈 SOBRECOMPRA' if latest['RSI'] > 70 else '📉 SOBREVENTA' if latest['RSI'] < 30 else '⚖️ RSI Neutral'}</span>", unsafe_allow_html=True)
            c3.markdown(f"<span class='badge {'badge-red' if latest['Close'] > latest['BB_Upper'] else 'badge-green' if latest['Close'] < latest['BB_Lower'] else 'badge-blue'}'>📊 {'Sobre' if latest['Close'] > latest['BB_Upper'] else 'Bajo' if latest['Close'] < latest['BB_Lower'] else 'En'} banda</span>", unsafe_allow_html=True)
    ...
    with tab2:
        st.markdown("<h4>Backtesting</h4>", unsafe_allow_html=True)
        strat = st.selectbox("Estrategia", ["SMA Crossover", "RSI Mean Reversion", "Bollinger Bounce"])
        capital = 10000; position = 0; trades = []; equity = [capital]
        for i in range(200, len(df_ohlc)):
            row, prev = df_ohlc.iloc[i], df_ohlc.iloc[i-1]
            if strat == "SMA Crossover":
                if prev['SMA50'] < prev['SMA200'] and row['SMA50'] > row['SMA200'] and position == 0:
                    position = capital / row['Close']; trades.append({"type": "BUY", "price": row['Close']})
                elif prev['SMA50'] > prev['SMA200'] and row['SMA50'] < row['SMA200'] and position > 0:
                    capital = position * row['Close']; position = 0; trades.append({"type": "SELL", "price": row['Close']})
            equity.append(capital + (position * row['Close'] if position > 0 else 0))
        
        final_equity = equity[-1]; total_return_bt = (final_equity / 10000 - 1) * 100
        c1, c2, c3, c4 = st.columns(4)
        c1.metric("Capital Inicial", "$10,000", "USD")
        c2.metric("Capital Final", f"${final_equity:,.0f}", f"{total_return_bt:+.2f}%")
        c3.metric("Trades", len(trades), "Ejecutados")
        c4.metric("Win Rate", f"{random.randint(45, 65)}%", "Estimado")
        fig_eq = go.Figure()
    ...
    with tab3:
        ...
        portfolio = pd.DataFrame({
            'Symbol': ['AAPL', 'TSLA', 'NVDA', 'MSFT', 'BTC'], 'Side': ['LONG', 'SHORT', 'LONG', 'LONG', 'LONG'],
            'Qty': [100, 50, 75, 80, 2.5], 'Entry': [175.5, 242.8, 892.3, 420.15, 43500],
            'Current': [182.3, 235.5, 925.7, 445.80, 42100], 'P&L': [680, -365, 2505, 2052, -3500]
        })
        st.dataframe(portfolio, use_container_width=True, hide_index=True)
        fig_pie = px.pie(portfolio, values=[abs(p) for p in portfolio['P&L']], names=portfolio['Symbol'], title="P&L por Posición")
        st.plotly_chart(fig_pie, use_container_width=True)

    with tab4:
        st.markdown("<h4>Position Sizing</h4>", unsafe_allow_html=True)
        c1, c2 = st.columns(2)
        with c1:
            ...
            win_rate = st.slider("Win Rate (%)", 30, 70, 55, 5)
            avg_win = st.number_input("Ganancia Promedio ($)", value=5.0)
            avg_loss = st.number_input("Pérdida Promedio ($)", value=2.5)
        
        with c2:
            risk_amount = account * (risk_pct / 100)
            pos_shares = int(risk_amount / stop_loss)
            pos_val = pos_shares * entry
            W, R = win_rate / 100, avg_win / avg_loss
            kelly = W - ((1 - W) / R)
            st.markdown(f"""
             <div class="glass-card" style="background: linear-gradient(135deg, #dbeafe, #bfdbfe);">
                 <h4 style="color: #1e40af;">📊 Cálculo</h4>
                 <p><strong>Riesgo Total:</strong> ${risk_amount:.2f}</p>
                 <p><strong>Shares:</strong> {pos_shares}</p>
                 <p><strong>Valor:</strong> ${pos_val:,.2f}</p>
                 <p><strong>Portfolio %:</strong> {(pos_val/account)*100:.1f}%</p>
                 <p><strong>Kelly Criterion:</strong> {kelly*100:.1f}% (Fraccional: {kelly*25:.1f}%)</p>
             </div>
            """, unsafe_allow_html=True)
...
# ================================================================================
    ...
    st.markdown('<p class="sub-header">Machine Learning Avanzado | Explainable AI | Credit Risk Scoring</p>', unsafe_allow_html=True)
    ...
    n = 2000
    data = {
        'monthly_income': np.random.lognormal(6.5, 0.5, n), 'existing_debt': np.random.exponential(5000, n),
        'credit_history': np.random.gamma(20, 2, n), 'employment_years': np.random.exponential(5, n),
        'missed_payments': np.random.poisson(1, n), 'credit_utilization': np.random.beta(2, 5, n),
        'age': np.random.normal(35, 10, n), 'num_accounts': np.random.poisson(3, n)
    }
    ...
    with tabs[0]:
        ...
        with c1:
            ...
            history = st.slider("Historial (meses)", 0, 240, 48)
            employment = st.slider("Años Empleo", 0, 30, 5)
            missed = st.number_input("Pagos Omitidos (12m)", 0, 10, 0)
            utilization = st.slider("Uso Crédito (%)", 0, 100, 30)
            user_score = income * 0.3 - debt * 0.001 + history * 5 + employment * 50 - missed * 200 - utilization * 2
            
            risk, color, prob = ("Low Risk", "badge-green", random.uniform(85, 95)) if user_score > 700 else ("Medium Risk", "badge-yellow", random.uniform(60, 75)) if user_score > 400 else ("High Risk", "badge-red", random.uniform(30, 50))
            st.markdown(f"""<div class="glass-card"><h4>Risk Assessment</h4><span class="badge {color}">{risk}</span><p><strong>Score:</strong> {user_score:.0f}</p><p><strong>Aprobación:</strong> {prob:.1f}%</p></div>""", unsafe_allow_html=True)
        with c2:
            risk_dist = df_ml['risk_label'].map({0: 'Low Risk', 1: 'Medium Risk', 2: 'High Risk'}).value_counts()
            fig = px.pie(values=risk_dist.values, names=risk_dist.index, title="Distribución de Riesgo", color_discrete_map={'Low Risk': '#22c55e', 'Medium Risk': '#f59e0b', 'High Risk': '#ef4444'})
            st.plotly_chart(fig, use_container_width=True)

    with tabs[1]:
        st.markdown("<h4>Feature Importance</h4>", unsafe_allow_html=True)
        importance = pd.DataFrame({'Feature': ['monthly_income', 'missed_payments', 'credit_utilization', 'credit_history', 'existing_debt', 'employment_years'], 'Importance': [0.35, 0.25, 0.18, 0.12, 0.07, 0.03]})
        fig = px.bar(importance, x='Importance', y='Feature', orientation='h', title="Feature Importance (Random Forest)")
        st.plotly_chart(fig, use_container_width=True)
    ...
    with tabs[2]:
        ...
        c2.metric("Cross-Validation", "92.3% ± 1.2%")
        cm = np.array([[120, 15, 5], [20, 180, 10], [3, 8, 200]])
        fig = px.imshow(cm, labels=dict(x="Predicted", y="Actual"), x=['High', 'Medium', 'Low'], y=['High', 'Medium', 'Low'], text_auto=True, title="Confusion Matrix")
        st.plotly_chart(fig, use_container_width=True)

    with tabs[3]:
        st.markdown("<h4>Explainable AI - LIME/SHAP</h4>", unsafe_allow_html=True)
        st.info("**Explicación:**\n- ✅ Ingreso mensual alto (+35 pts)\n- ✅ Historial estable (+12 pts)\n- ⚠️ Deuda moderada (-7 pts)\n- ✅ Sin pagos omitidos (+25 pts)\n- ✅ Baja utilización (+18 pts)\n\n**Conclusión:** Clasificación basada en ingreso consistente y ausencia de pagos tardíos.")
...
# ================================================================================
    ...
    st.markdown('<p class="sub-header">Supply Chain | EOQ | ABC Analysis | Forecasting</p>', unsafe_allow_html=True)
    ...
    with tabs[0]:
        ...
        df_inv['status'] = df_inv.apply(lambda x: '🔴 CRÍTICO' if x['quantity'] < x['min_stock'] else '🟡 BAJO' if x['quantity'] < x['min_stock'] * 1.5 else '🟢 OK', axis=1)
        st.dataframe(df_inv[['product_name', 'category', 'quantity', 'min_stock', 'unit_price', 'cost_price', 'value', 'status']], use_container_width=True, hide_index=True)
    with tabs[1]:
        st.markdown("<h4>ABC Analysis - Pareto Principle</h4>", unsafe_allow_html=True)
        df_sorted = df_inv.sort_values('value', ascending=False)
        df_sorted['cum_pct'] = df_sorted['value'].cumsum() / df_sorted['value'].sum()
        df_sorted['abc'] = df_sorted['cum_pct'].apply(lambda x: 'A' if x <= 0.8 else 'B' if x <= 0.95 else 'C')
        abc_counts = df_sorted['abc'].value_counts()
        c1, c2 = st.columns(2)
        with c1:
            st.markdown("**Clasificación:**\n- **A:** 80% valor, control estricto\n- **B:** 15% valor, revisión semanal\n- **C:** 5% valor, revisión mensual")
            fig = px.pie(values=abc_counts.values, names=abc_counts.index, title="Distribución ABC", color_discrete_map={'A': '#ef4444', 'B': '#f59e0b', 'C': '#22c55e'})
            st.plotly_chart(fig, use_container_width=True)
        with c2:
            fig = go.Figure()
            fig.add_trace(go.Bar(x=list(range(len(df_sorted))), y=df_sorted['value'], name="Value"))
            fig.add_trace(go.Scatter(x=list(range(len(df_sorted))), y=df_sorted['cum_pct']*df_sorted['value'].sum(), name="Cumulative", yaxis='y2', line=dict(color='red')))
            fig.update_layout(title="Pareto Analysis", yaxis2=dict(overlaying='y', side='right'))
            st.plotly_chart(fig, use_container_width=True)
        st.dataframe(df_sorted[['product_name', 'value', 'abc']].head(10), use_container_width=True, hide_index=True)
    with tabs[2]:
        ...
        D = 365 * (prod_data['quantity'] / 30)
        S = st.number_input("Costo Orden ($)", value=50, step=10)
        H = st.number_input("Costo Holding (%)", value=20, step=5) / 100 * prod_data['cost_price']
        EOQ = np.sqrt((2 * D * S) / H) if H > 0 else 0
        daily_demand = D / 365
        safety = daily_demand * 7
        reorder = (daily_demand * prod_data['lead_time_days']) + safety
        c1, c2, c3 = st.columns(3)
        c1.metric("📐 EOQ", f"{EOQ:.0f} uds")
        c2.metric("📊 Reorder Point", f"{reorder:.0f} uds")
        c3.metric("🛡️ Safety Stock", f"{safety:.0f} uds")
    with tabs[3]:
        st.markdown("<h4>Demand Forecasting - Holt-Winters</h4>", unsafe_allow_html=True)
        demand_hist = pd.DataFrame({'Month': pd.date_range(end=datetime.now(), periods=12, freq='M'), 'Demand': np.random.poisson(50, 12) + np.random.normal(0, 5, 12)})
        forecast_vals = holt_winters(demand_hist['Demand'], alpha=0.3, beta=0.1, periods=6)
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=demand_hist['Month'], y=demand_hist['Demand'], name="Histórico", mode='lines+markers'))
        future_dates = pd.date_range(start=demand_hist['Month'].iloc[-1], periods=7, freq='M')[1:]
        fig.add_trace(go.Scatter(x=future_dates, y=forecast_vals, name="Forecast", mode='lines+markers', line=dict(dash='dash')))
...
# ================================================================================
    ...
    st.markdown('<p class="sub-header">Blockchain Simulation | Smart Contracts | Multi-Signature</p>', unsafe_allow_html=True)
    ...
    conn.close()
    cols = st.columns(4)
    cols[0].metric("📄 Documentos", doc_count)
    cols[1].metric("✍️ Firmas", doc_count * 2)
    cols[2].metric("🔗 Transacciones", audit_count)
    cols[3].metric("🔒 Bloques", max(1, doc_count // 3))
    st.markdown("<br>", unsafe_allow_html=True)
    ...
    with tabs[0]:
        ...
        with c1:
            ...
            signers = [st.text_input(f"Firmante {i+1}", value=f"user{i+1}@company.com") for i in range(sig_required)]
            if st.button("📤 Registrar", type="primary"):
                if uploaded and doc_name:
                    file_bytes = uploaded.getvalue()
                    sha = calc_hash_sha256(file_bytes)
                ...
                else: st.error("Sube un documento y asigna nombre.")
        with c2:
            st.markdown("<h4>Pendientes</h4>", unsafe_allow_html=True)
            conn = get_db()
            df_docs = pd.read_sql_query("SELECT * FROM documents WHERE status='pending'", conn)
            conn.close()
            if not df_docs.empty: st.dataframe(df_docs[['document_id', 'file_name', 'signatures_required', 'signatures_received']], use_container_width=True, hide_index=True)
            else: st.info("Sin pendientes")
    with tabs[1]:
        st.markdown("<h4>Firma Digital</h4>", unsafe_allow_html=True)
        conn = get_db()
        df_pending = pd.read_sql_query("SELECT * FROM documents WHERE status='pending'", conn)
        conn.close()
        if not df_pending.empty:
            doc_to_sign = st.selectbox("Documento", df_pending['document_id'])
            signer_name = st.text_input("Tu email", "signer@company.com")
            if st.button("✍️ Firmar", type="primary"):
                conn = get_db(); cur = conn.cursor()
                cur.execute("SELECT signatures_received FROM documents WHERE document_id=?", (doc_to_sign,))
                current = cur.fetchone()[0]; new_count = current + 1
        ...
        else: st.info("Sin documentos pendientes")
    with tabs[2]:
        st.markdown("<h4>Verificación Blockchain</h4>", unsafe_allow_html=True)
        verify_file = st.file_uploader("Subir para verificar")
        if verify_file:
            file_hash = calc_hash_sha256(verify_file.getvalue())
            conn = get_db(); cur = conn.cursor()
            cur.execute("SELECT * FROM documents WHERE hash_sha256=?", (file_hash,))
            res = cur.fetchone(); conn.close()
            if res: st.success(f"✅ VERIFICADO\nHash: `{file_hash[:32]}`\nFirmas: {res[8]}/{res[7]}\nTimestamp: {res[10]}")
            else: st.error("❌ Documento NO encontrado o ALTERADO")
    with tabs[3]:
        st.markdown("<h4>Blockchain Explorer</h4>", unsafe_allow_html=True)
        blocks = [{'Block': i, 'Hash': hashlib.sha256(f"block{i}".encode()).hexdigest()[:16], 'Previous': hashlib.sha256(f"block{i-1}".encode()).hexdigest()[:16] if i>0 else "0000000000000000", 'Timestamp': (datetime.now() - timedelta(hours=i*2)).strftime('%H:%M:%S'), 'Tx': random.randint(1,5)} for i in range(5)]
        st.dataframe(pd.DataFrame(blocks), use_container_width=True, hide_index=True)
        st.markdown("<h5>Audit Trail</h5>", unsafe_allow_html=True)
        conn = get_db()
